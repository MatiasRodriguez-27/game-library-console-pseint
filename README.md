...
Funcion VerificarCapturaRojas(mat Por Referencia, vectorCaptura Por Referencia)
    Dimension vectorAux[100, 3]  // Para almacenar m�ltiples opciones de captura
        ...
        Para columna <- 0 Hasta 7 Con Paso 1 Hacer
            Si mat[fila, columna] = "R" Entonces
                ...
                Si (fila-2 >= 0 y columna-2 >= 0 y (mat[fila-1, columna-1] == "N" o mat[fila-1, columna-1] == "n") y mat[fila-2, columna-2] == " ") Entonces
                    ...
                    vectorAux[numCapturas, 1] <- fila + 1  // m�s 1 para que quede igual que las otras filas y columnas de las funciones Movimiento
    ...
    Si numCapturas > 1 Entonces
        // Pedir al usuario que elija cu�l captura desea usar
        Escribir "Hay m�ltiples opciones de captura. Elija una:"
        Para opcion <- 1 Hasta numCapturas Con Paso 1 Hacer
            Escribir opcion, ". Pieza en (", vectorAux[opcion-1, 1], ", ", vectorAux[opcion-1, 2], ")"
        FinPara
		
        Repetir
            Escribir "Ingrese el n�mero de su opci�n: "
            Leer opcion
...
Funcion MovimientoR (mat Por Referencia)
	...
	si vectorCaptura[0] == 1 Entonces
		...
		escribir "Pieza Roja (", fila, ";", columna, ") est� obligada a comer"
		
		si mat[fila-1, columna-1] == "R" Entonces
			repetir
				Escribir "�D�nde desea mover la pieza? (primero fila, luego columna): "
				esEntero = 0
				Mientras(esEntero == 0) Hacer
					leer auxChar
					esEntero = verificarSiEntero(auxChar)
					...
					SiNo
						Escribir "ERROR: el valor ingresado para fila no es un n�mero entero"
					FinSi
				FinMientras
				...
				Mientras(esEntero == 0) Hacer
					...
					SiNo
						Escribir "ERROR: el valor ingresado para columna no es un n�mero entero"
					FinSi
				FinMientras
			...
			SI (Abs(deltaX) == 2 Y Abs(deltaY) == 2) Entonces
				...
				SI (mat[capturarY-1, capturarX-1] == "N" O mat[capturarY-1, capturarX-1] == "n") Entonces
					...
					// Actualizar posici�n de la pieza movida
					SI (mat[fila - 1, columna - 1] == "R") Entonces
	...
	SiNo
		repetir
			repetir
				Escribir "Seleccione la posici�n de la pieza que desea mover (primero fila, luego columna): "
				esEntero = 0;
				Mientras(esEntero==0) Hacer
					leer auxChar
					esEntero = verificarSiEntero(auxChar);
...
Funcion MovimientoN (mat Por Referencia)
	...
	si vectorCaptura[0] == 1 Entonces
		fila = vectorCaptura[1]
		columna = vectorCaptura[2]
		escribir "Pieza Negra (", fila, ";", columna, ") est� obligada a comer"
...
Funci�n Finjuego <- VerificarFinJuego(mat Por Referencia,Finjuego por valor,turnoColor por valor)
	...
	SiNo
		si(turnoColor==1) Entonces
			Para fila <- 0 Hasta 7 Con Paso 1 Hacer
				Para columna <- 0 Hasta 7 Con Paso 1 Hacer
					Si mat[fila,columna] == 'R' Entonces
						// Verificar si hay un movimiento v�lido para Rojas
		...
		si (turnoColor==2) Entonces
			Para fila <- 0 Hasta 7 Con Paso 1 Hacer
				Para columna <- 0 Hasta 7 Con Paso 1 Hacer
					Si mat[fila,columna] == 'N' Entonces
						// Verificar si hay un movimiento v�lido para Negras
...
Si Finjuego <> 0 Entonces
	...
	SiNo
		...
		SiNo
			Escribir "No hay m�s movimientos posibles para el jugador �Empate!"
		FinSi
...
Funci�n jugarAhorcado
    ...
    para x = 0 Hasta n-1 Con Paso 1 Hacer
        vector1(x) = Subcadena(secreta, x , x )  // Ajustamos el �ndice correctamente
        vector2(x) = "_"
		Borrar Pantalla
    FinPara
    ...
    Mientras a < 5 Hacer
        ...
        sino 
            ...
            si a == 5 Entonces
				...
				Escribir "        #"
				Escribir "# # # # # # # # #"
                Escribir "No te quedan m�s intentos"
            FinSi
        FinSi
    FinMientras
Fin Funci�n
